...
9. `[CLI]` **compose video** `--framed --subtitles --audio speakers --bgm "<G1で選んだジャンル>"` → 本編mp4。
10. **intro-builder スキル**: イントロ生成（台本/服装非重複/尺/QAは intro-builder が判断）。生成物を見せ**自動で次へ**。`publish intro-compose` 未実装なら仕上げ合成はskip/手動と明示。
11. `[CLI]` **[L]**: 章からサムネ文言を作り `publish thumbnail --top/--bottom`／タイトル・要約を書き `publish description`。見せて**自動で次へ**。
